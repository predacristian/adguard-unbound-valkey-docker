...
# Test SET operation
if ! redis-cli set test_key "Hello, Redis!" > /dev/null; then
...
if ! redis-cli exists test_key > /dev/null; then
    echo "Redis EXISTS operation failed"
    exit 1
fi

# Test DEL operation
if ! redis-cli del test_key > /dev/null; then
...
redis-cli ping | grep -q 'PONG' && echo "Response time test passed"
