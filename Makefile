...
# Docker Compose command - auto-detect v2 (docker compose) or v1 (docker-compose)
COMPOSE := $(shell docker compose version >/dev/null 2>&1 && echo "docker compose" || echo "docker-compose")

# Container name (can be overridden: make up CONTAINER_NAME=my-dns-stack)
...
health: ## Check health status
	...
	@docker inspect --format='{{.State.Health.Status}}' $(CONTAINER_NAME) 2>/dev/null || echo "$(RED)Container not running$(NC)"
...
test: build ## Run all tests (builds image first, includes integration tests)
	...
	@docker exec $(CONTAINER_NAME) /tests/test_valkey.sh || (echo "$(RED)Valkey tests failed$(NC)" && $(COMPOSE) down && exit 1)
	@echo ""
	@echo "$(BLUE)Running AdGuard tests...$(NC)"
	@docker exec $(CONTAINER_NAME) /tests/test_adguard.sh || (echo "$(RED)AdGuard tests failed$(NC)" && $(COMPOSE) down && exit 1)
	@echo ""
	@echo "$(BLUE)=== Running Integration Tests ===$(NC)"
	@echo "$(BLUE)Running cache integration tests...$(NC)"
	@docker exec $(CONTAINER_NAME) /tests/test_cache_integration.sh || (echo "$(RED)Cache integration tests failed$(NC)" && $(COMPOSE) down && exit 1)
	@echo ""
	@echo "$(BLUE)Running E2E query tests...$(NC)"
	@docker exec $(CONTAINER_NAME) /tests/test_e2e_query.sh || (echo "$(RED)E2E tests failed$(NC)" && $(COMPOSE) down && exit 1)
	@echo ""
	@echo "$(BLUE)Running ad blocking tests...$(NC)"
	@docker exec $(CONTAINER_NAME) /tests/test_ad_blocking.sh || (echo "$(RED)Ad blocking tests failed$(NC)" && $(COMPOSE) down && exit 1)
	@echo ""
	@echo "$(BLUE)Running DoT tests...$(NC)"
	@docker exec $(CONTAINER_NAME) /tests/test_dot.sh || echo "$(YELLOW)DoT tests skipped (not configured)$(NC)"
	@echo ""
...
test-bats: build ## Run BATS integration tests
	...
	@docker exec $(CONTAINER_NAME) bats /tests/integration.bats || (echo "$(RED)BATS tests failed$(NC)" && $(COMPOSE) down && exit 1)
...
test-integration: ## Run only integration tests (container must be running)
	@echo "$(BLUE)Running integration tests...$(NC)"
	@docker exec $(CONTAINER_NAME) /tests/test_cache_integration.sh && \
...
test-dot: ## Run only DoT tests (container must be running)
	@echo "$(BLUE)Running DoT tests...$(NC)"
	@docker exec $(CONTAINER_NAME) /tests/test_dot.sh
...
quick-test: ## Quick smoke test without rebuild (uses existing image)
	...
	@docker exec $(CONTAINER_NAME) /tests/test_unbound.sh && \
	 docker exec $(CONTAINER_NAME) /tests/test_valkey.sh && \
	 docker exec $(CONTAINER_NAME) /tests/test_adguard.sh && \
	 echo "$(GREEN)✓ All tests passed!$(NC)" || (echo "$(RED)✗ Tests failed$(NC)" && $(COMPOSE) down && exit 1)
	@$(COMPOSE) down
